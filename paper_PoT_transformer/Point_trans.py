...
class PointNet_Vit(nn.Module):
    ...
    def __init__(self, seq_len, num_classes=8, embed_dim=16, depth=2,
    ...
    def forward_pn_features(self, point_data, target_data):
        # linear embedding for point_data
        sample_fea = np.array([])
        ...
        x=sample_fea.transpose(1,0)  # 从（batch_size, self.seq_len,attriNum)->(self.seq_len, batch_size, attriNum)
        x=x.squeeze()
        # add class token
