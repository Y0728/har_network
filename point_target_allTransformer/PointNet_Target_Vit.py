...
from pointnet_utils_PAT_allTrans_tf_vit import PointNetEncoder, feature_transform_reguliarzer
...
class Seq2Seq_by_Transformer(nn.Module):
    # def __init__(self,embed_dim=512, depth=12,num_heads=8, mlp_ratio=4., qkv_bias=True, representation_size=None, distilled=False,
    #              drop_rate=0., attn_drop_rate=0., drop_path_rate=0.,  norm_layer=None,
    #              act_layer=None, weight_init=''):
    def __init__(self, seq_len, class_token = True,embed_dim=512, depth=12,num_heads=8, mlp_ratio=4., qkv_bias=True, representation_size=None, distilled=False,
        ...
        # self.seq2seq_transformer = Seq2Seq_by_Transformer()
        # embed_dim = 512, depth = 12, num_heads = 8, mlp_ratio = 4., qkv_bias = True, representation_size = None, distilled = False,
        # drop_rate = 0., attn_drop_rate = 0., drop_path_rate = 0., norm_layer = None,
        # act_layer = None,
...
class PointNet_Target_Vit(nn.Module):
    """ Vision Transformer
    ...
    def __init__(self, num_layers, hidden_size,seq_len, k = 5, normal_channel = True, model = 'rnn',img_size=224, patch_size=16, in_chans=3, num_classes=8, embed_dim=512, depth=12,
        ...
        self.seq2seq_transformer_1 = Seq2Seq_by_Transformer(embed_dim=256, seq_len = 20,class_token=False,depth=4)
        self.seq2seq_transformer_2 = Seq2Seq_by_Transformer(embed_dim=512,seq_len = 20,depth=8)
    ...
    def forward_pn_features(self, point_data, target_data):
        ...
        embeded_target_data = self.seq2seq_transformer_1(embeded_target_data)
        ...
        if self.dist_token is None:
            ...
            x = torch.cat((cls_token, self.dist_token.expand(x.shape[0], -1, -1), x), dim=1)
        return self.seq2seq_transformer_2(x), trans_fea_array
        # x = self.pos_drop(x + self.pos_embed)
        # x = self.blocks(x)
        # x = self.norm(x)
        # if self.dist_token is None:
        #     print('!!!!!')
        #     return self.pre_logits(x[:, 0]), trans_fea_array
        # else:
        #     return x[:, 0], x[:, 1], trans_fea_array
...
@torch.no_grad()
def _load_weights(model: PointNet_Target_Vit, checkpoint_path: str, prefix: str = ''):
...
def _create_vision_transformer(variant, pretrained=False, default_cfg=None, **kwargs):
    ...
    model = build_model_with_cfg(
        PointNet_Target_Vit, variant, pretrained,
