...
def parse_args():
    ...
    parser.add_argument('--model', default='PointNet_Target_Vit', help='model name [default: pointnet_cls]')
...
def main(args):
    ...
    classifier = model.PointNet_Target_Vit( num_layers = 2, hidden_size = 2048, k = num_class, normal_channel=args.use_normals, model=args.rnn_type,seq_len = seqLen)
    ...
    for epoch in range(start_epoch, args.epoch):
        with SummaryWriter(log_dir=res_dir) as writer:
            writer.add_scalar('result/acc', best_class_acc,epoch)
