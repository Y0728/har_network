...
from tmp import train_test_split, HARDataset
...
def parse_args():
    ...
    parser.add_argument('--activity_list', default=['run'], help='activity types') #['fall', 'run']',,'stand','jump','sit','fall','run','walk','bend'
...
def main(args):
    ...
    classifier = model.PointNet_Target( num_layers = 2, hidden_size = 1024, k = num_class, normal_channel=args.use_normals, seq_len = seqLen)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        with torch.no_grad():
            instance_acc, class_acc_array, val_loss = test(classifier.eval(), testDataLoader, num_class=num_class)
