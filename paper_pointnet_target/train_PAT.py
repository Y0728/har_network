...
from tmp import train_test_split, HARDataset,dataset_exists
...
def parse_args():
    ...
    parser.add_argument('--model', default='PointNet_Target', help='model name [default: pointnet_cls]')
    ...
    parser.add_argument('--activity_list', default=['stand','jump','sit','fall','run','walk','bend'], help='activity types') #['fall', 'run']',,'stand','jump','sit','fall','run','walk','bend'
    #train ({2: 7005, 5: 5534, 6: 3795, 0: 3760, 1: 3306, 3: 3278, 4: 928})
    #test {2: 2264, 0: 1267, 6: 1161, 1: 1155, 3: 1090, 5: 904, 4: 291}
    ...
    parser.add_argument('--data_source', default='pat', help='can be point or target')
...
def test(model, loader, model_file,num_class=40):
    mean_correct = []
    class_acc = np.zeros((num_class, 3))  # 这里声明了
    classifier = model.eval()

    criterion = importlib.import_module(model_file).get_loss()  # 加，为了计算val_loss
    ...
    # for j, (points, y) in tqdm(enumerate(loader), total=len(loader)):

        if not args.use_cpu:
            points, target, y, all_preds, all_truth = points.cuda(), target.cuda(), y.cuda(), all_preds.cuda(), all_truth.cuda()
            # points, y = points.cuda(), y.cuda()
        if args.pointLSTM == False or args.seq_len==1:
...
def main(args):
    if args.data_source=='point':
        model_file='PointNet'
    elif args.data_source=='pat':
        model_file= 'PointNet_Target'
    # else:
    #     model_file='PaT_trans'
    ...
    file_handler = logging.FileHandler('%s/%s.txt' % (log_dir, model_file))
    ...
    if file_exists := dataset_exists(activity_list, args.seq_len, args.concat_frame_num, 'trainwoSeq'):
    ...
    train_dataset = HARDataset(data_path, 'point', activity_list, concat_framNum=args.concat_frame_num,seq_len=args.seq_len,
                               file_list=train_files_point, pointLSTM=args.pointLSTM,dataset_type='trainwoSeq')
    ...
    test_dataset = HARDataset(data_path, 'point', activity_list, concat_framNum=args.concat_frame_num,seq_len=args.seq_len,
                              file_list=test_files_point, pointLSTM=args.pointLSTM,dataset_type='testwoSeq')
    ...
    shutil.copy('./%s.py' % model_file, str(exp_dir))
    shutil.copy(f'./{os.path.basename(__file__)}', str(exp_dir))
    classifier = model.PointNet_Target(  num_classes = num_class, normal_channel=args.use_normals)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        with torch.no_grad():
            instance_acc, class_acc_array, val_loss, cf, total_correct = test(classifier.eval(), testDataLoader, model_file, num_class=num_class)

            class_acc = np.mean(class_acc_array[np.where(class_acc_array[:, 1] != 0), 2])  #加，以前在test()里边，为了单独计算class_acc
    ...
    # percetage
    percetage = np.array([[cf[i][j] / cf[i].sum() for j in range(len(cf[i]))] for i in range(len(cf))])
    f1, ax1 = plt.subplots()
    ...
    plt.show()
    logger.info(f'The confusion matrix with percetage:\n{percetage}')
