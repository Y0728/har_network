...
def parse_args():
    ...
    parser.add_argument('--activity_list', default=['stand','jump','sit','fall','run','walk','bend'], help='activity types') #['fall', 'run']',,'walk','stand','fall','bend'
    ...
    parser.add_argument('--data_source', default='target', help='can be point or target')
...
def test(model, loader, model_file, num_class=40):
    ...
    for j, (points, target, y) in tqdm(enumerate(loader), total=len(loader)):
        ...
        if model_file=='Point_SaT':
            pred, trans_feat = classifier(points, target)
        elif model_file=='Target_SaT':
            pred = classifier( target)
...
def main(args):
    if args.data_source == 'point':
        model_file = 'Point_SaT'
    elif args.data_source == 'target':
    ...
    classifier = model.PointNet_Vit( num_classes = num_class, normal_channel=args.use_normals, seq_len = seqLen,embed_dim=64)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for batch_id, (points, target, y) in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader),
            ...
            if model_file=='Target_SaT':
                pred = classifier(target)
                loss = criterion(pred, y.long())
            elif model_file=='Point_SaT':
    ...
    sns.set()
    f, ax = plt.subplots()
    sns.heatmap(cf, annot=True, ax=ax,cmap='BuPu',fmt='d',xticklabels=activity_list,yticklabels=activity_list)  # 画热力图
